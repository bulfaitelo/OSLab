                            ...
                            <div class="modal-body">
                                ...
                                <div class="repeater-container">
                                    <div class="row">
                                        <div class="col-md-1">
                                            <div class="form-group">
                                                <label for="p"> Parcela </label>
                                                {!! html()->text('p', 1)->class('form-control')->placeholder('Valor total da despesa')->required()->disabled() !!}
                                            </div>
                                        </div>
                                      <div class="col-md-4">
                                        <div class="form-group">
                                          <label for="valor_parcela"> Valor Parcela </label>
                                          {!! html()->text('valor_parcela')->class('form-control decimal')->placeholder('Valor total da despesa')->required() !!}
                                        </div>
                                      </div>
                                      <div class="col-md-3">
                                        <div class="form-group">
                                          <label for="vencimento_parcela"> Vencimento Parcela </label>
                                          {!! html()->date('vencimento_parcela')->class('form-control')->placeholder('Nome da forma de pagamento') !!}
                                        </div>
                                      </div>
                                      <div class="col-md-4">
                                        <div class="form-group">
                                ...
                                {{-- <div class="repeater">
                                    ...
                                    <div class="row">
                                        <div class="col-md-1">
                                            <div class="form-group">
                                                <button id="addRepetitions" type="button" class=" btn btn-block btn-primary"><i class="fas fa-plus"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                </div> --}}
...
{{-- <script>
  ...
  <script>
    // Função para adicionar repetições do formulário
    function addRepetitions(numRepetitions) {
      var formContainer = $('.repeater-container');
      var pValue = parseInt($('#p').val(), 10); // Valor inicial do campo "p"
      var vencimento = $('#vencimento').val();
      var vencimentoDate = moment(vencimento);
      var regex = /[.,\s]/g;
      var valor = $('#valor').val()
      var valorTradado = (parseFloat(valor.replace(regex, ''))) / numRepetitions;



      // Remova as repetições atuais
      formContainer.children('.row:not(:first)').remove();

      // Adicione as novas repetições
      for (var i = 1; i < numRepetitions; i++) {
        var newForm = formContainer.children('.row:first').clone();
      pValue++; // Incrementa o valor do campo "p" em cada repetição
      vencimentoDate.add(1, 'M');
      newForm.find('#p').val(pValue); // Define o novo valor do campo "p"
      newForm.find('#vencimento_parcela').val(vencimentoDate.format('YYYY-MM-DD'));
      newForm.find('#valor_parcela').val(valorTradado); // Define o novo valor do campo "p"
    ...
    $(document).ready(function () {
      // Adicione um evento de escuta no input "parcelas"
      $('#parcelas').on('input', function () {
        var numRepetitions = parseInt($(this).val(), 10);

        // Certifique-se de que o número de repetições seja um valor válido
        if (isNaN(numRepetitions) || numRepetitions < 1) {
          numRepetitions = 1;
        }

        // Adicione as novas repetições do formulário
        addRepetitions(numRepetitions);
      });
    });
  </script>
<script>
